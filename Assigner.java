...
public class Assigner {
    ...
    public static void assignValue(Vector<String> vector, boolean isConst,boolean isArray) {
        ...
        if (vector.elementAt(Main.pointer).equals("Assign")) {
            Main.pointer++;
            String res = ExpAnalyzer.expAnalyze(vector, isConst, false);
            ...
            if (ident != null) {
                if (ident.infos.elementAt(0).isConst) System.exit(-4);
                    ...
                    if (ident.infos.elementAt(0).level == 0) {
                        Main.res.add("store i32 " + res + ", i32* @x" + ident.infos.elementAt(0).no);
                    } else
                        Main.res.add("store i32 " + res + ", i32* %x" + ident.infos.elementAt(0).no);
                }
            } else System.exit(-5);
            ...
            Main.counter++;
           String res = ExpAnalyzer.expAnalyze(vector,isConst, false);
           Main.res.add("store i32 " + res + ", i32* %x" + p);
    ...
    public static void assignArray(Vector<String> vector, Info info, Boolean isGlobal, Boolean isConst, int mount) {
        ...
        if (isGlobal) {//全局数组，初值可计算，注意判断isConst
            ...
            if (isConst) {
                s = ("@x" + (info.no) + " = dso_local constant [" + mount + " x i32] [");
            } else {
                s = ("@x" + (info.no) + " = dso_local global [" + mount + " x i32] [");
            }
            for (int i = 0; i < mount; i++) {
                if (i < mount - 1)
            ...
            //Main.res.add("%x" + info.no + " = alloca [" + mount + " x i32]");
            Main.res.add("%x" + Main.counter + " = getelementptr ["+info.length+" x i32], ["+info.length+" x i32]* %x" + info.no + ", i32 0 , i32 0");
            Main.res.add("call void @memset(i32* %x" + Main.counter + ", i32 0, i32 " + mount * 4 + ")");
            for (int i = 0; i < mount; i++, Main.counter++) {
                Main.res.add("%x" + Main.counter + " = getelementptr ["+info.length+" x i32], ["+info.length+" x i32]* %x" + info.no + ", i32 0, i32 " + i);
                Main.res.add("store i32 " + arrayVal.elementAt(i) + ", i32* %x" + Main.counter);
            ...
            for (int i = 0; i < mount; i++, Main.counter++) {
                Main.res.add("%x" + Main.counter + " = getelementptr ["+info.length+" x i32], ["+info.length+" x i32]* %x" + info.no + ", i32 0 , i32 " + i);
                Main.res.add("store i32 " + arrayVal2.elementAt(i) + ", i32* %x" + Main.counter);
            }
        }
    }
            ...
            for (int ii = 0; ii < divs[totaldiv - 1]; ii++) {
                if (isGlobal || isConst) {
                    ...
                    if (vector.elementAt(Main.pointer).equals("RBrace")) {
                        ...
                        String val = ExpAnalyzer.expAnalyze(vector, false, false);
                        arrayVal2.add(val);
                        length++;
                        if(!vector.elementAt(Main.pointer).equals("RBrace"))Main.pointer++;//跳过 ","
                    }
